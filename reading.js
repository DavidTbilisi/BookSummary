// reading.js - interactive controls for book pages
(function(){
  ...
  bar.innerHTML = `
    ...
    <button type="button" data-act="theme" title="Toggle theme">Theme</button>
    <button type="button" data-act="top" title="Back to top">Top</button>
    <button type="button" data-act="print" title="Print page">Print</button>
  `;
  ...
  function adjustFont(delta){
    const current = prefs.fontSize || 1.05; // rem
    const next = Math.min(1.6, Math.max(0.85, +(current + delta).toFixed(2)));
    prefs.fontSize = next; savePrefs(); applyPrefs();
  }
  function toggleLayout(){
    prefs.layout = (prefs.layout === 'columns') ? 'single' : 'columns';
    if (prefs.layout === 'single') delete prefs.layout; // default single
    savePrefs(); applyPrefs();
  }
  const themes = ['sepia','dark','night',''];
  function toggleTheme(){
    const current = prefs.theme || '';
    const idx = themes.indexOf(current);
    const next = themes[(idx + 1) % themes.length];
    prefs.theme = next; if(!next) delete prefs.theme; savePrefs(); applyPrefs();
  }
  ...
  applyPrefs();
  updateProgress();
  setTimeout(updateProgress, 100); // ensure correct after images load
})();
